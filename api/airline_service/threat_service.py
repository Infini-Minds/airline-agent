...
def allocate_hotels(session, affected_crews, affected_passengers, airport_code):
    ...
    def _generate_voucher_ref():
        return (
            f"V{''.join(random.choices(string.ascii_uppercase + string.digits, k=6))}"
        )
...
def create_bomb_threat_disruption(
    session, city: str, airport_codes: list[str]
) -> Disruption:
...
def get_flights_to_handle(
    session, airport_codes: list[str], alternate_airport: str | None
):
    ...
    for i, flight in enumerate(flights):
        new_status, new_dest = _decide_flight_action(
            flight, airport_codes, alternate_airport
        )
...
def process_city_bomb_threat(city: str, alternate_airport: str | None = None):
    ...
    try:
        with session.begin():
            ...
            # 3. Handle flights
            flights, flight_disruptions, affected_flight_ids = get_flights_to_handle(
                session, airport_codes, alternate_airport
            )
...
def get_dashboard_map_data(session):
    ...
    for d in disruptions:
        ...
        lat, lon = resolve_airport_location(
            airport.airport_code, airport.city, airport.country
        )
