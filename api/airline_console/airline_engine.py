...
def get_airports_by_city(session: Session, city: str):
    return session.query(Airport).filter(
...
def get_bookings_by_flight_ids(session: Session, flight_ids: list[int]):
    return session.query(PassengerBooking).filter(
        PassengerBooking.flight_id.in_(flight_ids)
    ).all()


def get_crews_and_aircraft(session: Session, flight_ids: list[int]):
    crews = session.query(CrewAssignment).filter(
        CrewAssignment.flight_id.in_(flight_ids)
    ).all()

    aircraft_ids = session.query(Flight.aircraft_id).filter(
        Flight.flight_id.in_(flight_ids)
    ).distinct().all()
    
    aircraft_ids = [a[0] for a in aircraft_ids]

    aircrafts = session.query(Aircraft).filter(
        Aircraft.aircraft_id.in_(aircraft_ids)
    ).all()
...
def close_airports(session: Session, airport_codes: list[str]):
    airports = session.query(Airport).filter(
        Airport.airport_code.in_(airport_codes)
    ).all()
...
def resolve_disruption(session: Session, disruption_id: str):
    disruption = session.query(Disruption).filter_by(
        event_id=disruption_id
    ).first()
    if disruption:
...
def get_allocated_crews_and_aircraft(session: Session, flight_ids: list[int]):
    ...
    aircrafts = session.query(Aircraft).filter(
        Aircraft.flight_id.in_(flight_ids)
    ).all()
...
def get_available_hotels(session: Session, airport_code: str):
    ...
    """
    return session.query(HotelDetails).filter(
