...
if incident_feed_data is None and map_data is None:
    st.warning(
        "Waiting for API connection. Ensure Flask server is running on port 5000."
    )
else:
    ...
    with col_filter:
        ...
        status_filter = st.selectbox(
            "Filter by Status", options=status_options, key="status_select_key"
        )
    ...
    with col_download:
        ...
        full_df = (
            pd.DataFrame(incident_feed_data) if incident_feed_data else pd.DataFrame()
        )
